...
export default function DoctorChatsPage() {
  ...
  const markSelectedChatMessagesDeliveredLocally = (messageIds = []) => {
    ...
    patchSelectedChatMessages((messages) =>
  ...
  const emitVisibleSeenMessages = () => {
    ...
    if (
      !container ||
      !selectedChat?.chat?.messages?.length ||
  ...
  const loadChat = async (patientId) => {
    ...
    try {
      ...
      if (
        requestSeq === chatRequestSeqRef.current &&
        selectedPatientIdRef.current === String(patientId)
      ) {
  ...
  useEffect(() => {
    ...
    const handleDetailUpdated = ({ patientId, payload } = {}) => {
      if (
        !payload ||
        String(patientId || "") !== String(selectedPatientIdRef.current || "")
      )
        return;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      await axiosInstance.post(
        `/patient-chats/doctor/${selectedPatientId}/messages`,
        formData,
        {
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(
      CHAT_SEEN_STORAGE_KEY,
      JSON.stringify(seenIncomingAtByPatient),
    );
  }, [seenIncomingAtByPatient]);

  const getEntryByPatientId = (patientId) =>
    patients.find((entry) => entry._id === patientId);

  const getEntryLastMessageTime = (entry) => {
    if (entry?.lastMessageAt) return new Date(entry.lastMessageAt).getTime();
    if (entry?.lastMessage?.createdAt)
      return new Date(entry.lastMessage.createdAt).getTime();
  ...
  const getAttachmentPreviewText = (entry) => {
    ...
    const attachments = Array.isArray(entry?.lastMessage?.attachments)
      ? entry.lastMessage.attachments
      : [];
  ...
  const getVoiceAttachmentLabel = (attachment) => {
    ...
    return /^voice-note-\d+\.(webm|ogg|mp3|wav|m4a|aac)$/i.test(fileName)
      ? "Voice message"
      : fileName;
  };

  const selectedListPatient = patients.find(
    (entry) => String(entry?._id) === String(selectedPatientId),
  );
  ...
  const patient = isSelectedChatAligned
  ...
  const newChats = sortedPatients.filter(
    (entry) => getEffectiveUnreadCount(entry) > 0,
  );
  const otherChats = sortedPatients.filter(
    (entry) => getEffectiveUnreadCount(entry) === 0,
  );
    ...
    <div className="grid gap-3 xl:grid-cols-[340px_1fr] xl:gap-4">
      {showPatientList ? (
        <aside className="rounded-3xl border border-[var(--color-border)]/80 bg-[var(--color-card)]/95 p-3 shadow-[0_10px_40px_-10px_rgba(93,112,82,0.15)] sm:p-4">
          ...
          <p className="mt-1 text-xs text-[var(--color-text-secondary)]">
            Patients with chat access enabled.
          </p>
          ...
          <div className="mt-4 space-y-3 max-h-[72vh] overflow-y-auto pr-1">
            {isLoading ? (
              ...
              <>
                    ...
                    <div className="space-y-2">
                      {newChats.map((entry) => (
                        <button
                          ...
                          style={(() => {
                            const isSelected =
                              String(selectedPatientId) === String(entry._id);
                            return {
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="rounded-full bg-[var(--color-primary)] px-2 py-0.5 text-[10px] font-semibold text-white">
                                {getEffectiveUnreadCount(entry) > 9
                                  ? "9+"
                                  : getEffectiveUnreadCount(entry)}
                              </span>
                            </div>
                          </div>
                          <p className="mt-1 text-xs text-[var(--color-text-secondary)]">
                          ...
                          <p className="mt-1 text-[10px] font-medium text-[var(--color-text-primary)]">
                            {getAttachmentPreviewText(entry)}
                          </p>
                        </button>
                      ))}
                    </div>
                ...
                {otherChats.length > 0 ? (
                  <div>
                    <p className="mb-2 text-[10px] font-semibold uppercase tracking-[0.25em] text-[var(--color-text-secondary)]">
                      All chats
                    </p>
                    <div className="space-y-2">
      ...
      {showChatPanel ? (
        <section className="flex min-h-[70vh] h-[calc(100vh-9rem)] flex-col rounded-2xl border border-[var(--color-border)]/80 bg-[var(--color-card)]/95 shadow-[0_10px_40px_-10px_rgba(93,112,82,0.15)]">
          {!patient ? (
            <div className="m-auto p-6 text-center rounded-2xl text-sm text-[var(--color-text-secondary)]">
            ...
            <>
              <div className="border-b border-[var(--color-border)]/80 bg-[var(--color-card)]/90 px-4 py-3 rounded-2xl sm:px-5 sm:py-4">
                ...
                <p className="text-xs text-[var(--color-text-secondary)]">
                  {patient.chatUsername} · {patient.chatInviteStatus}{" "}
                  {patient.chatLastLoginAt
                    ? `· last login ${formatShortDate(patient.chatLastLoginAt)}`
                    : ""}
                </p>
              </div>
