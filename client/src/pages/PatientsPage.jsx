...
function SectionLabel({ text }) {
  return (
    <p className="text-xs font-bold uppercase tracking-widest mb-3" style={{ color: "#475569" }}>{text}</p>
  );
}

// ─── Tag Input ────────────────────────────────────────────────────────────────

function TagInput({ value, onChange, onAdd, onRemove, items, placeholder, color = "#10B8A9" }) {
  return (
    <div>
      <div className="flex gap-2 mb-3">
        ...
        <button onClick={onAdd}
          ...
          style={{ background: `rgba(${color === "#10B8A9" ? "16,184,169" : "239,68,68"},0.12)`, color }}>
          + Add
        </button>
      </div>
      {items.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {items.map((item, i) => (
            <span key={i} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium"
              style={{ background: `rgba(${color === "#10B8A9" ? "16,184,169" : "239,68,68"},0.1)`, border: `1px solid rgba(${color === "#10B8A9" ? "16,184,169" : "239,68,68"},0.2)`, color }}>
              {item}
              <button onClick={() => onRemove(i)} className="ml-0.5 opacity-70 hover:opacity-100"
                style={{ color: "#ef4444", fontSize: "10px" }}>✕</button>
            </span>
          ))}
        </div>
      )}
    ...
    <div className="max-w-2xl mx-auto px-1">
      ...
      <div className="flex items-center gap-3 mb-6 px-4 py-3 rounded-2xl" style={S.card}>
        ...
        <div>
          ...
          <p className="text-xs" style={{ color: "#64748b" }}>{patient.age} yrs · {patient.gender} · {patient.bloodGroup}</p>
        </div>
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-2xl p-5" style={S.card}>
          ...
          <TagInput value={diseaseInput} onChange={setDiseaseInput} onAdd={addDisease}
        ...
        <div className="rounded-2xl p-5" style={S.card}>
          ...
          <div className="space-y-4">
                ...
                {medicines.map((med, i) => (
                  <div key={i} className="p-4 rounded-xl" style={S.section}>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <div>
                        <label className="block text-xs mb-1" style={{ color: "#64748b" }}>Dosage *</label>
                      ...
                      <div>
                        ...
                        <select value={med.frequency} onChange={(e) => updateMedicine(i, "frequency", e.target.value)}
                          className={inputCls} style={S.input} onFocus={focusInput} onBlur={blurInput}>
                          <option value="" style={{ background: "#0a1628" }}>Select frequency</option>
            ...
            <div>
              ...
              <TagInput value={labInput} onChange={setLabInput} onAdd={addLabTest}
                onRemove={(i) => setLabTests((p) => p.filter((_, idx) => idx !== i))}
                items={labTests} placeholder="e.g. CBC, Blood Sugar, Urine Test" color="#a78bfa" />
            </div>
          </div>
        </div>
...
function PatientDetailPage({ patient, onBack, onNewCheckup, onCheckupAdded }) {
          ...
          <div key={checkup._id} className="rounded-2xl overflow-hidden" style={S.card}>
            ...
            <div className="flex items-center justify-between px-5 py-4"
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-white">Last Check-up Disease: [{checkup.prescription?.diagnosis || "No diagnosis"}]</p>
                    ...
                    {checkup.prescription.medicines.map((med, i) => (
                      <div key={i} className="flex items-center gap-3 px-4 py-3 rounded-xl"
                        ...
                        <div className="w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold flex-shrink-0"
                          style={{ background: "rgba(16,184,169,0.15)", color: "#10B8A9" }}>
                          {i + 1}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-semibold text-white">
                            {med.name}
                            <span className="font-normal ml-2" style={{ color: "#64748b" }}>— {med.dosage}</span>
                          </p>
...
function AddPatientForm({ onBack, onAdded }) {
  ...
  const addHistory = () => {
    ...
    setForm((p) => ({ ...p, medicalHistory: [...p.medicalHistory, historyInput.trim()] }));
    setHistoryInput("");
  };
    ...
    <div className="max-w-2xl mx-auto px-1">
      ...
      <div className="rounded-2xl p-5 sm:p-6 space-y-5" style={S.card}>
        ...
        <div>
          ...
          <TagInput value={historyInput} onChange={setHistoryInput} onAdd={addHistory}
...
export default function PatientsPage() {
  const [view, setView] = useState("list"); // list | add | detail | checkup
    ...
    <div>
      ...
      <div className="rounded-2xl overflow-hidden" style={S.card}>
        ...
        <div className="hidden sm:grid grid-cols-5 gap-4 px-5 py-3"
          ...
          {["Patient", "Age & Gender", "Phone", "Blood Group", "Added"].map((h) => (
        ...
        {!isLoading && patients.map((patient) => (
          <div key={patient._id} onClick={() => openPatient(patient)}
            ...
            <div className="sm:hidden flex items-center gap-3 px-4 py-4">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <button
                  ...
                  className="w-8 h-8 rounded-lg flex items-center justify-center transition-all hover:bg-red-500 hover:bg-opacity-15"
            ...
            <div className="hidden sm:grid grid-cols-5 gap-4 items-center px-5 py-4">
              ...
              <span className="text-xs px-2.5 py-1 rounded-full w-fit font-semibold"
                style={{ background: "rgba(16,184,169,0.1)", color: "#10B8A9" }}>
                {patient.bloodGroup}
              </span>
