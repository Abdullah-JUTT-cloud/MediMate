...
import { RefreshCcw, Siren, Stethoscope, Trash2 } from "lucide-react";
import toast from "react-hot-toast";
import axiosInstance from "../api/axios";
import { RowSkeleton } from "../components/SkeletonLoaders";
import ConfirmDialog from "../components/ConfirmDialog";
import useConfirmDialog from "../hooks/useConfirmDialog";
import BookAppointmentWithPatientSearch from "../components/patients/BookAppointmentWithPatientSearch";
...
export default function AppointmentsPage({
  ...
  // The previous `view === "book"` inline form has been removed. The
  // reschedule-from-cancelled flow now opens the same modal that the
  // "+ Book Appointment" button does, so booking UI lives in one
  // place (`BookAppointmentWithPatientSearch` → `BookAppointmentModal`).
  // The branch is left as a no-op safety net in case any future caller
  // accidentally routes through `view = "book"`.
  ...
  return (
    ...
    {isBookModalOpen && (
      <BookAppointmentWithPatientSearch
        onClose={() => setIsBookModalOpen(false)}
        onBooked={async (appointment) => {
          handleBooked(appointment);
          // Preserve the reschedule-from-cancelled side effect that the
          // old inline form did inline in its `handleSubmit`. The
          // modal doesn't know about `rescheduleCancelledAppointmentId`,
          // so we own the call from here.
          if (rescheduleCancelledAppointmentId && onEmergencyRescheduleComplete) {
            try {
              ...
              toast.error("Reschedule cleanup failed");
            }
          }
