    ...
    <div className="relative min-h-screen overflow-hidden bg-[#FDFCF8] text-[#2C2C24]">
      ...
      <main className="relative z-10 px-4 pb-20 pt-28 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-5xl">
          <p className="font-mono text-[11px] font-semibold uppercase tracking-[0.24em] text-[#5D7052]">Product capabilities</p>
          ...
          <div className="mt-12 grid gap-5">
            {features.map((feature) => (
              <section key={feature.title} className="rounded-4xl border border-[#DED8CF]/70 bg-[#FEFEFA]/95 p-6 shadow-[0_10px_40px_-10px_rgba(93,112,82,0.12)] sm:p-8">
                <h2 className="font-heading text-2xl font-semibold">{feature.title}</h2>
                <ul className="mt-4 space-y-3 text-[#78786C]">
                  {feature.details.map((item) => (
                    <li key={item} className="flex items-start gap-3 leading-relaxed">
                      <span className="mt-1.5 h-2 w-2 rounded-full bg-[#5D7052]" />
                      <span>{item}</span>
                    </li>
                  ))}
                </ul>
          ...
          <section className="mt-12 rounded-4xl border border-[#DED8CF]/70 bg-[#FEFEFA]/95 p-6 shadow-[0_10px_40px_-10px_rgba(193,140,93,0.16)] sm:p-8">
            ...
            <div className="mt-5 grid grid-cols-1 gap-4 md:grid-cols-2">
              {outcomes.map((outcome) => (
                <div key={outcome} className="rounded-3xl border border-[#DED8CF]/80 bg-[#F0EBE5]/45 p-4 text-sm leading-relaxed text-[#78786C]">
                  {outcome}
                </div>
              ))}
            </div>
