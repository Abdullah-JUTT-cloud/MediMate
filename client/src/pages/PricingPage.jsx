...
export default function PricingPage() {
  ...
  const visibleFeatures = showAllFeatures ? PLAN_FEATURES : PLAN_FEATURES.slice(0, 8);
      ...
      <main className="relative z-10 px-4 pb-20 pt-20 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <section className="text-center">
            <p className="mb-2 text-sm font-medium text-[var(--color-primary)]">Simple pricing. No surprises.</p>
            <p className="font-mono text-[11px] font-semibold uppercase tracking-[0.24em] text-[var(--color-primary)]/70">Pricing</p>
            <h1 className="mt-3 font-heading text-4xl font-semibold leading-tight sm:text-5xl">One complete plan. Simple billing.</h1>
          ...
          <section className="relative mx-auto mt-16 max-w-3xl rounded-4xl border border-[var(--color-border)]/70 bg-[var(--color-card)]/95 p-8 shadow-[0_15px_40px_rgba(0,0,0,0.08)] transition-all duration-500 hover:shadow-[0_20px_50px_rgba(0,0,0,0.12)] md:p-10">
            ...
            <div className="mx-auto mb-8 flex max-w-fit items-center rounded-full border border-[var(--color-border)]/80 bg-[var(--color-bg-soft)]/60 p-1">
              ...
              >
                <span>Annual</span>
                <span className={`rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-tighter ${billingCycle === "annual" ? "bg-white/20 text-white" : "bg-[var(--color-primary)]/10 text-[var(--color-primary)]"}`}>
                  Save 20%
                </span>
              </button>
            </div>
            ...
            <div className="mt-8 flex flex-col items-center justify-center">
              <div className="flex items-baseline gap-1">
                <span className="text-6xl font-extrabold tracking-tight text-[var(--color-text-primary)]">{pricing.price}</span>
                <span className="text-xl font-medium text-[var(--color-text-secondary)]/70">{pricing.period}</span>
              </div>
              <p className="mt-3 text-center text-xs font-semibold uppercase tracking-[0.18em] text-[var(--color-secondary)]">{pricing.helper}</p>
            </div>
            ...
            <div className="mt-10 rounded-3xl border border-[var(--color-border)]/80 bg-[var(--color-bg-soft)]/45 p-6 md:p-8">
              ...
              <ul className="mt-6 space-y-4">
                {visibleFeatures.map((item) => (
                  <li key={item} className="flex items-start gap-4 text-sm leading-relaxed text-[var(--color-text-secondary)]">
                    ...
                    <span>
                      {item.includes("prescriptions") || item.includes("WhatsApp") || item.includes("emergency") ? (
          ...
          <section className="mt-20 rounded-4xl border border-[var(--color-border)]/70 bg-[var(--color-card)]/95 p-6 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.05)] sm:p-10">
            <div className="flex flex-wrap items-center justify-between gap-6 border-b border-[var(--color-border)]/50 pb-8">
              <div>
                ...
                <p className="mt-2 text-[var(--color-text-secondary)]">Everything you need to know about MedAlerto</p>
              </div>
              <Link
                to="/faq"
              ...
              {faqs.map((item, index) => (
                ...
                >
                  ...
                  >
                    ...
                    <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-[var(--color-border)] bg-white/50 transition-transform duration-300 ${openFaqIndex === index ? "rotate-45 border-[var(--color-primary)]/40" : ""}`}>
                      <span className="text-xl font-medium text-[var(--color-text-primary)]">+</span>
                    </div>
                  </button>
                  <div 
                    id={`faq-answer-${index}`}
