    ...
    <div className="relative min-h-screen overflow-hidden bg-[#FDFCF8] text-[#2C2C24]">
      ...
      <main className="relative z-10 px-4 pb-20 pt-28 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-5xl">
          <p className="font-mono text-[11px] font-semibold uppercase tracking-[0.24em] text-[#5D7052]">Support</p>
          ...
          <div className="mt-12 grid grid-cols-1 gap-5 md:grid-cols-3">
            {channels.map((channel) => (
              <section key={channel.title} className="rounded-4xl border border-[#DED8CF]/70 bg-[#FEFEFA]/95 p-6 shadow-[0_10px_40px_-10px_rgba(93,112,82,0.12)]">
                <h2 className="font-heading text-xl font-semibold">{channel.title}</h2>
                <p className="mt-3 text-sm leading-relaxed text-[#78786C]">{channel.description}</p>
                <a
                  ...
                  className="mt-5 inline-block text-base font-bold text-[#5D7052] hover:underline"
                >
                  {channel.contactLabel}
                </a>
                <p className="mt-3 text-xs text-[#78786C]">{channel.meta}</p>
              </section>
            ))}
          </div>
          ...
          <p className="mt-8 text-sm text-[#78786C]">
